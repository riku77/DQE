...
function AttackTester() {
    ...
    if ($('.is-spawn').val() == "true") {
        ...
        if ($('.st-enemyHP').val() >= 0) {
            ...
            $('.btnLi').html('<li><a href="../ゾーマ/ゾーマ.html">次のボスへ</a></li><li><a href="../../index.html">TOPへ戻る</a></li>')
    ...
    if ($(".st-enemyHP").val() <= 1000) {
        document.getElementById('message').innerHTML = '<span class="message">' + $(".st-enemyName").val() + "は" + "ベホマを使った" + '</span>'
        $(".st-enemyHP").attr('value', Number($('.st-enemyHP').val()) + 300);
        $(".st-enemyHP").html($(".st-enemyHP").val());
