...
function AttackTester() {
    ...
    if ($('.is-spawn').val() == "true") {
        ...
        if ($('.st-playerHP').val() >= 0) {
            ...
            $('.btnLi').html('<li><a href="ゾーマ.html">再戦</a></li><li><a href="../../index.html">TOPへ戻る</a></li>')
            endWindow.classList.add("show");
    ...
    if (Number($(".st-enemyState").val()) == 1) {
        document.getElementById('message').innerHTML = '<span class="message">' + $(".st-enemyName").val() + "は" + "闇の衣を纏っている" + '<br>' + $(".st-playerName").val() + "の攻撃を無効化した" + '</span>'
        $(".st-enemyHP").attr('value', Number(8000));
        $(".st-enemyHP").html($(".st-enemyHP").val());
    }

    if ($(".st-enemyHP").val() <= 1000) {
        document.getElementById('message').innerHTML = '<span class="message">' + $(".st-enemyName").val() + "は" + "マヒャデドスを唱えた" + "<br>" + "700ダメージを与えた" + '</span>'
        $(".st-playerHP").attr('value', Number($('.st-playerHP').val()) - 700);
...
function light_Ball() {
    document.getElementById('message').innerHTML = '<span class="message">' + $(".st-playerName").val() + "は光の玉を使った" + "<br>" + "大魔王ゾーマの闇の衣が消えた" + '</span>'
    $(".st-enemyState").attr('value', Number(0));
    $(".st-enemyState").html("状態:通常");
}
