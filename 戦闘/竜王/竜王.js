...
function AttackTester() {
    ...
    if ($('.is-spawn').val() == "true") {
        ...
        if ($('.st-enemyHP').val() >= 0) {
            ...
            $('.btnLi').html('<li><a href="../シドー/シドー.html">次のボスへ</a></li><li><a href="../../index.html">TOPへ戻る</a></li>')
    ...
    if ($(".st-enemyHP").val() <= 1000) {
        document.getElementById('message').innerHTML = '<span class="message">' + $(".st-enemyName").val() + "は" + "闇のブレスを使った" + '<br>' + "500ダメージを与えた" + '</span>'
        $(".st-playerHP").attr('value', Number($('.st-playerHP').val()) - 500);
...
function activeMenu_message_view(id) {
    ...
    switch (id) {
        ...
        case 4:
            ...
            break;
        case 5:
            document.getElementById('message').innerHTML = 'どうぐ</span>';
            break;
        case 6:
